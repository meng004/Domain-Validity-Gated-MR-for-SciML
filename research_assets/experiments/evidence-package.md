...
| `research_assets/runs/same-domain-variant-primary-workflow/same_domain_variant_primary_workflow_report.json` (+ rubric decisions and referenced raw ledgers) | observed | Same-domain second checkpoint-family primary workflow for the S4/S5 cylinder-flow MGN configuration variants. Full rubric-to-verdict package: node permutation passes 2/2, mirror-y OOD-stress fails 60/60 over S4/S5 x 3 held-out trajectories x 10 frames, reference-relative conservation passes 54/54 while absolute conservation remains deferred, and exact synthetic symmetric-mesh mirror-y fails 6/6. This is real cylinder-flow variant-family evidence, not PhysicsNeMo/EchoWave, cross-dataset, reliability, or broad-generalization evidence. |
| `research_assets/runs/pointmlp-cylinder-primary-workflow/pointmlp_cylinder_primary_workflow_report.json` (+ checkpoint, ledgers, raw `.npz`) | observed | Different-architecture cylinder-flow SUT: a newly trained non-MGN PointMLP row-wise coordinate network. Train trajectories 0--1, held-out trajectory 2; node permutation 9/9 pass, mirror OOD 10/10 fail, conservation diagnostic 9/9 pass, exact synthetic symmetry 3/3 fail, median rollout rel L2 0.0298. Not PhysicsNeMo/EchoWave, not production CFD, not cross-dataset reliability evidence. |
...
| `C26-same-domain-variant-primary-workflow` | observed | May describe the S4/S5 same-domain full primary workflow package and its 2/2 node-permutation passes, 60/60 mirror-y OOD-stress failures, 54/54 reference-relative conservation passes, and 6/6 exact symmetric-mesh failures. | May appear as same-domain MeshGraphNet configuration-variant evidence only; not as PhysicsNeMo/EchoWave, cross-dataset, reliability, or absolute-conservation evidence. |
| `C27-pointmlp-cylinder-primary-workflow` | observed | May describe the newly trained PointMLP cylinder-flow workflow and its 9/9 node-permutation passes, 10/10 mirror OOD failures, 9/9 conservation-diagnostic passes, 3/3 exact-symmetry failures, and median rollout rel L2 0.0298. | Different architecture-family cylinder-flow SUT only; not PhysicsNeMo/EchoWave, production CFD, cross-dataset, reliability, or absolute-conservation evidence. |
...
## PhysicsNeMo MGN Object-A smoke workflow (scoped, evidence-gated)

- A minimal production-framework artifact chain now executes for Object A: `tools/run_physicsnemo_mgn_smoke_workflow.py` imports NVIDIA PhysicsNeMo `MeshGraphNet`, stages first complete official DeepMind `cylinder_flow` train/test TFRecord records outside git, trains a new CPU checkpoint, and commits the checkpoint metadata, raw source/follow-up outputs, rubric decisions, and metric ledgers under `research_assets/runs/production-grade-sut-extension/physicsnemo-mgn-vortex-shedding/`.
- The admitted representation relation passes on the smoke subset (node-permutation relative L2 `0.0`). Rollout accuracy, mirror-y OOD stress, and reference-relative conservation diagnostic ledgers are recorded, but the result is smoke-subset evidence only.
- Evidence-gating takeaway: the production-framework artifact chain is executable for Object A at smoke scale, but this is not yet a full production-scale PhysicsNeMo benchmark and gives no AeroGraphNet, DoMINO, cross-dataset, reliability, or geometry-independent validity claim.
...
- Full-scale production external-SUT or cross-dataset pass/fail rates; the PhysicsNeMo MGN Object-A result is a smoke-subset production-framework execution only, the S4/S5 package is a same-domain MeshGraphNet configuration-variant workflow, and mirror-y on asymmetric held-out meshes is asserted only as an approximate OOD-stress probe, not an exact relation for those meshes.
...
1. Extend beyond the current MeshGraphNet same-domain variants to a genuinely separate cylinder-flow implementation/checkpoint family (e.g. PhysicsNeMo or EchoWave) before any cross-SUT or geometry-independent rate.
