...
- More than one external architecture/dataset has been evaluated for cylinder-flow; the added S4/S5 package is a same-domain MeshGraphNet configuration-variant workflow, and mirror-y on asymmetric held-out meshes is asserted only as an approximate OOD-stress probe, not an exact relation for those meshes.
