...
- [ ] P0c: 生产级外部 SUT extension（Task 1 feasibility audit + Task 2 candidate ledgers + Task 2.5 runtime staging + Task 2.6 MGN official data/checkpoint staging attempt + Task 2.7 official access/source staging + Task 2.8 complete DeepMind TFRecord staging 已完成但 blocked；artifacts include `feasibility_report.json`、per-object `candidate_ledger.json`、`physicsnemo_runtime_staging_report.json`、`physicsnemo_mgn_asset_staging_report.json`、`official_access_staging_report.json`；Object-A official data blocker 已关闭，但下一步仍需 official/new PhysicsNeMo checkpoint/API、raw outputs、metric ledgers 后才能进入 Task 3-5 workflows）
