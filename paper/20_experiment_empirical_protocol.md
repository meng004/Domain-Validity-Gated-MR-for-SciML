...
一般性的真实 SUT 通过率或失效率，而是把可观察证据、限定性证据和 blocked 证据分层。
...
PR4 后，另有一组 artifact-gated real-SUT pilots 已经 observed：node permutation、
mirror-y OOD-stress、discrete-divergence diagnostic。它们只支持 one SUT、one checkpoint、
one/few MR、one/few frames 的限定结论。三条 METBENCH_MGN_* planned SUT execution
仍然 blocked，因为当前没有记录对应的 `METBENCH_MGN_DATA_ROOT`、`METBENCH_MGN_REPO`、
`METBENCH_MGN_CHECKPOINT`、精确命令和输出产物。
...
| Artifact-gated pilot | Node permutation on one real MeshGraphNets checkpoint | observed scoped pilot | Manifest, raw outputs, metric ledger, claim ledger |
| Artifact-gated pilot | Mirror-y approximate OOD-stress on one real MeshGraphNets checkpoint | observed bounded within-SUT frame rate | Manifest, raw outputs, metric ledger, claim ledger, out-of-relation-domain record |
| Artifact-gated pilot | Discrete-divergence reference-relative diagnostic on one real MeshGraphNets checkpoint | observed scoped pilot | Manifest, raw outputs, metric ledger, conservation report |
...
判定规则采用 fail-closed policy：任何真实 SUT claim 缺少 dataset、repo、checkpoint、
command 或 output artifact 时，claim 维持 blocked。Fixture verdict 只能支持 asset path
observed，不能外推为 SUT behavior。Artifact-gated pilot 只有在 manifest、raw outputs、
metric ledger 和 claim ledger 都存在并通过校验时，才允许写成限定 Results。
...
|---|---|
| observed | Artifact exists and directly supports the narrow statement being made. |
...
Blocked claims 不能写成 Results。Observed pilot claims 只能按其证据边界写入 Results。
它们不能外推为 reliability、accuracy、baseline、cross-SUT、exact mirror-y 或
geometry-independent 结论。
...
三个 METBENCH 计划的真实 SUT execution 仍 blocked。这些 blocked claims 不能写成
Results；只能作为实验协议、证据门槛和后续实验计划来呈现。PR4 的 mirror-y
10-frame 结果只解除一个 bounded within-SUT frame-level OOD-stress claim，并且仍是
single-SUT / single-MR / single-case pilot；它不解除上述 blocked claims。
