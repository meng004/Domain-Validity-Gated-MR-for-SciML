# 78 · Superseded NEXT_STEPS history snapshot

Date: 2026-07-03

Purpose: preserve the pre-Phase-6 live NEXT_STEPS history before shrinking NEXT_STEPS.md to the current JSS execution track. This is an archive only; do not execute old TOSEM/IST/RESS plans from this file.

---

# NEXT_STEPS — MR识别/圆柱绕流 (JSS regular paper 稳定接收主线)

> Last updated: 2026-07-03 by Codex · 当前主投已由用户改定为 **Journal of Systems and Software (JSS), regular paper**。`paper/75` 和 `paper/76` 已将 TOSEM 从当前主投降为 aspirational ceiling；`paper/77` 是 JSS 稳定接收执行计划。
...
- **JSS 官方契约（2026-07-03 核验）**：JSS scope 覆盖 software engineering、V&V/testing、AI in SE、SE for AI systems；要求 claims 有 evidence，可为 empirical studies、simulation、formal proofs 或 other validation；single-anonymized；abstract ≤250 words；keywords 1–7；highlights 必交，3–5 条且每条 ≤85 characters；LaTeX editable source 可用。
- **当前最高优先级**：Phase 0/1。先冻结 JSS source-of-truth 并重跑 evidence/test gates，分类历史 IST/TOSEM 守卫与真实 JSS 阻断项；再进入 title/abstract/introduction 正文修订。
...
## 历史记录：2026-07-02 以前 TOSEM/IST/RESS 决策已被 2026-07-03 JSS 决策覆盖
...
- **✅ Phase C 骨架已交付（2026-07-01，`paper/68`）**：thesis 收敛到「数值可判定性可采性判据」单一贡献；新 thesis/标题候选/贡献重排（C1 floor gate+定理独大、C2 MR-card / C3 typed-verdict 降支撑、C4 必要性演示）/弃增量卖点清单/摘要骨架/章节重排图（operator-floor 与 measured-advantage 提到最前）。**gate 过**：核心问题 + novelty delta 各一句锁定。**待用户拍板 3 项**：①标题三选一 ②§2.4 可靠性段缩短 vs 删 ③确认 C1 独大后进 Phase B。
- **🔵 next-action**：拍板 `paper/68` §8 三项 → 启动 **Phase B**（唯一枢轴=C1(ii) operator-floor bound 一般化到非结构网格 soundness 定理）。
...
- **复核(2026-06-30)**：对照 RESS 真实发表谱(Crossref issn 0951-8320)——蜕变测试 0 先例；每篇 ML-surrogate/CFD/PINN/软件-V&V 都桥接到可靠性/风险/诊断量；最近邻先例=核反应堆保护系统安全关键软件测试(Lee RESS2020)。**persona 未过严，desk-reject 风险被发表谱坐实。**
- **多 LLM 异源面板(2026-06-30，`paper/57`)**：gpt-5.5/grok-4.3/gemini-3.5/Claude 四方独立**均 Reject** 现稿。**桥接 ROI 结论：唯一高 ROI 路径=(4) physical-consistency NN → fault diagnosis（RESS scope 明列该主题，本文 typed verdict 已 ~80% 是诊断分诊）**。轻档=纯重构+诊断输出层(复用 seeded-fault，但证据薄、必要不充分)；**强档=核 point-kinetics/OpenMC 系统在环 + RPS 误警/漏检率量（呼应 Lee2020，CNNC 署名）→ 中等新增执行实验，能真正满足 RESS**。其余桥接：(1) Med/Med 有包装风险、(2) Low-Med 缺退化物理、(3) Low 最不现实。**不愿做核实验则改投 SE。**
- **桥接(4)强档最小可执行方案(2026-06-30，`paper/58`)**：勘察姊妹库证实核资产已就绪——PKE **PINN 代理 + mutant_catalog（fault_class 已分 physical_parameter_fault/numerical_tolerance_fault/equivalent_control = 三分诊断金标准）** + OpenMC C5G7 kill matrix（部分已只读导入本仓）+ 本仓 `run_cross_program_coverage.py` runner。**新增=把 gate 新执行为故障分诊 + 算 FAR/MDR(误警/漏检率,带 Wilson CI) + accuracy-only 基线对照 + RPS trip 决策层 + 正文重写**；无新重型训练。工作量**中(周级)**。核心卖点=validity gate 把 numerical_tolerance_fault 正确归数值伪影→降 FAR(系统在环可靠性量)。**待用户认可工作量后启动；不愿做则 SE。第 0 步：清点 mutant 计数确认统计功效。**
- **方案 B：嫁接 RESS 已发表 SUT+可靠性量(2026-06-30，`paper/59`)**：scope 契合**优于**方案 A(核)。**最优=PINN-PDEM 可靠性**(Das2023 ress.2023.109849 等)：PINN 解 GDEE 得 PDF→失效概率；**概率守恒 ∫p=1=本文守恒 MR 最干净可采实例**，operator-floor→求积地板，MR 适用性理论原样迁移；失效概率=RESS 最经典量；**可不依赖原作者代码、用标准 PDEM 基准复现**。次优=WDN 漏损 GNN(Fink ress.2025.111494，节点质量守恒 MR→漏损定位，开放基准)。不契合=Kriging 结构可靠性(g(x) 无物理 MR)。代价 vs A：多一次标准小模型训练；收益：scope by-construction、可复现强。**最强=B 主(PINN-PDEM)+ 现有 CFD/PINN/FNO 作 breadth。待定夺：B-PDEM(推荐)/B-WDN/A-核/A+B。**
- **方案 B 执行启动(2026-06-30，`paper/60`)**：用户拍板 B=PINN-PDEM 主 + Fink WDN 补 + 现有资产作 breadth。**资产勘察**：本仓 PINN 工具链(`train_pinn_diffusion2d/burgers2d`、`run_pinn_*`)已在→PDEM 数据本地生成可即启(无 blocker)；本机无 WDN 数据。**WDN 数据已授权获取并 clone 到 `~/Codes/wdn-external/`**(不入本仓 git)：`BattLeDIM`(L-TOWN_v2 .inp + competition_leakages 金标准) + `physics_informed_gnns_for_wds`(AAAI 开放码, **trained_models/l_town 预训练**, 物理信息=守恒, 本文守恒 MR 审计它)。Fink 专属码未公开→引其作锚、用开放栈。**主线 MR=概率守恒∫p=1→失效概率 P_f 可信度; 补线 MR=节点质量守恒→漏损 FAR/MDR。下一步：PDEM 主线第 0 步(选 SDOF 基准+本仓 PINN 解 GDEE 最小原型)。**
...
- **✅ §12 IST 合规**:结构化 abstract 五段 296≤300;Highlights 5 条均 ≤85(已抽出单独文件 `submissions/IST/highlights.txt`);Keywords 7;Title 7 词;Data avail / CRediT / CoI / GenAI 声明 / Funding(NSFC 12575176 等)全在;wordcount **12158 ≤ 15000**(余 2842)。
...
- **✅ cover letter 初稿**:`submissions/IST/cover_letter.md`(突出 novelty + industrial relevance + 诚实 scope;作者信息/建议审稿人留占位)。剩余投稿系统逻辑项:Editorial Manager 元数据、可选 graphical abstract(禁 AI 生成图)。
