...
|---|---|---|---|---|
| PC1-domain-validity-rubric | Supported as method/design claim | The paper proposes a domain-validity rubric that screens candidate MRs before execution. | `research_assets/rubric/domain_validity_rubric.json`; `manuscript/manuscript.md`; `submissions/IST/main.tex` | The rubric proves physical validity or replaces domain expertise. |
| PC2-mr-card-executable-assets | Supported as asset/workflow claim | Retained candidates are represented as MR cards and executable assets with transformations, metrics, tolerances, exclusions, and verdicts. | `research_assets/mr_cards/`; `tools/validate_research_assets.py`; `manuscript/manuscript.md` | Every card has cross-SUT empirical evidence. |
...
| PC7-llm-candidate-support-only | Supported as process boundary | LLM use is restricted to candidate generation and material organization; it does not judge MR validity. | `manuscript/manuscript.md`; `submissions/IST/main.tex`; method and ethics sections | LLMs automatically identify valid SciML MRs. |
...
| PC10-seeded-fault-detection | Observed | The paper's MRs as detectors catch 5/10 injected mutants and localize by MR class (continuity->boundary/scale, symmetry->physical-channel/adjacency; node-permutation none, exact-by-design). | `research_assets/runs/seeded-fault-detection/raw/metric_ledger.json`; `claim-ledger.yml` (C10) | A general or real-world fault-detection rate, validated localization model, reliability, or cross-SUT claim. |
