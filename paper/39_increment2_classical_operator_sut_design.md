...
> **云端执行修正（2026-06-15，实事求是）**：上表 `nonconservative_form` 原拟用**中心**平流
> 形式 `u_i·(u_{i+1}−u_{i−1})/2Δx`。云端首跑实测它 |dM|max=2.22e-16（与 baseline 同为舍入级,
> **未检出**）——因为周期边界下 `Σ_i u_i(u_{i+1}−u_{i−1}) = Σu_i u_{i+1} − Σu_i u_{i-1} ≡ 0`
> （移位重标精确抵消），中心平流形式**恰好守恒一阶矩(总质量)到机器精度**,它破坏的是能量而非质量。
> 按铁律「只修缺陷构造、不动 tol」,改用**迎风(后差,此处场全程为正故为正确迎风方向)原始变量平流**
> `u_i·(u_i−u_{i−1})/Δx`——同样是真实的非守恒形式 CFD bug(把方程写成原始变量迎风而非通量形式),
> 实测 |dM|max=1.78e-3 ≫ tol,**真实破坏质量守恒**。中心平流的盲区作为诚实注记保留(见 §4 要点:
> 守恒 MR 是破坏 flux-difference telescoping 结构的**结构检测器**,非任意非守恒离散都抓)。
