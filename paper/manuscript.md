...
This paper investigates how physically meaningful metamorphic relations (MRs) can be screened and operationalized as executable oracle-free test assets for scientific machine-learning systems. The central question is when a candidate relation may be trusted as a test: we argue an MR is admissible only when its verdict tolerance provably dominates the intrinsic error floor of the discrete operator that measures it, a floor with a characterizable truncation order rather than an empirically guessed threshold. The goal is not to improve a predictive model, but to make the step from candidate relation to executable test asset systematic, auditable, and explicit about the physical, numerical, and software conditions under which each relation holds. Cylinder-flow surrogates across four architectures are the concrete case study.
...
This paper makes three scoped contributions, evaluated through a cylinder-flow case study examined across four architectures. The conceptual core is a single idea the prior literature does not contain: **an MR is admissible only when its verdict tolerance dominates the intrinsic error floor of the discrete operator that measures it** — a floor that is a property of the measurement, with a characterizable truncation order, not of the model, the data, or the test designer. Two devices follow from it — a measurement-floor admissibility gate and a typed domain-inadmissibility verdict — and a third element tests them against seeded faults. The positive claim is methodological; the MR-class-to-fault-class diagnostic is used as stress-test evidence, not a validated localization model.

**Measurement-floor admissibility gate.** A candidate MR is retained only if its tolerance can be shown to exceed the intrinsic error floor of the operator that scores it — for a P1 discrete-divergence operator on a triangular mesh, the $O(h)$ truncation floor in the mesh spacing. This is categorically different from the two nearest tolerance ideas in the literature: Eniser et al.'s *relaxations* [eniser2022relaxations] derive a tolerance empirically from stochastic RL rollouts, and MetaTrimmer [duqueTorres2023metaTrimmer] derives MR constraints data-drivenly from random-input violation logs. Both read tolerance off observed behaviour; ours reads it off the *numerical analysis of the scoring operator*, so a relation can be refused before any model is run — as happens here for absolute conservation, whose floor is uncalibratable on the deployed mesh. The gate is implemented as a domain-validity rubric and MR-card executable-asset format recording physical basis, preconditions, boundary compatibility, output mapping, metric, tolerance, and exclusion rules. In this study the floor magnitude is estimated from the reference field on the deployed mesh; a closed-form a-priori bound is left to future work. To our knowledge, grounding an MR tolerance in the measurement operator's own truncation floor is new in the SciML setting.

**Typed domain-inadmissibility verdict.** The verdict is read in two dimensions — relation-violation against domain-violation magnitude — and, unlike the binary bug-vs-inapplicability separation of the Duque-Torres line [duqueTorres2023bugornot, duqueTorres2023completePipeline, duqueTorres2023metaTrimmer], the inadmissibility axis is *typed*: its sub-dimensions are drawn from PDE-domain preconditions, geometry, boundary conditions, and operator admissibility, and the axis carries a continuous per-relation score computed from committed precondition measurements rather than a skip/proceed flag. Calibration of these scores across MR classes is left to future work.

**Seeded-fault diagnostic stress test.** As an element none of the closest prior works addresses, the paper's own MRs are used as fault detectors against an independently re-implemented seeded-fault catalogue, showing a by-class response pattern: the continuity MR responds to boundary-condition and normalization-scale faults; the symmetry MR responds to physical-channel and mesh-adjacency faults; node-permutation equivariance, exact by design under these faults, responds to none. This is suggestive evidence for a typed MR-class-to-fault-class diagnostic, but it remains a stress test over seeded faults rather than a localization model.

To keep the contribution boundary easy to follow: the three devices above are the claims; Sections 5.3-5.6 carry their primary evidence; Section 5.8 shows the predicate transferring across model families; everything else (baselines, witness audits) is context, not claim.
...
A second objection reads the deferred absolute-conservation relation as a gap in the end-to-end pipeline — one of three primary MR classes without a full executable verdict. The portfolio answers this directly: the conservation class does produce full executable verdicts wherever its measurement floor is calibratable (the FNO periodic conservation MR fails 24/24 against a calibrated case-level floor; the PINN reference-relative conservation passes 6/6), and the gate refuses execution precisely on the one variant — MGN open-boundary absolute conservation — where a verdict could not be separated from the operator's own discretization error. A pipeline that executed that relation anyway would have produced the false conservation pass discussed below.
