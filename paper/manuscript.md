...
**Unified catalogue and statistics.** Phase 3 merges 10 executed canonical MGN mutants, 2 executed adversarial MGN mutants, 24 closed-form output-level PINN probes, and 24 closed-form output-level FNO probes into a 60-entry unified fault catalogue (see §5.1). The artifact reports precision/recall with Wilson intervals: node-permutation 1.00/1.00, conservation 1.00/0.81, and mirror-y 0.94/0.55; it also reports Wilcoxon and Cliff's δ effect-size summaries, including PINN MR-B diffusion-vs-Burgers δ = 0.78 (large) with paired Wilcoxon p = 0.5 at n = 3. The PINN and FNO entries are closed-form probes with no retraining, not retrained mutant checkpoints. The same by-class detection structure --- each MR localizing one fault class, with a structural blind subspace surviving when a fault enters the suite's own invariances --- recurs across four architecture families: the trained-mutant MeshGraphNets roster, a converged PointMLP coordinate network (union 4/8 of the applicable catalogue, Wilson 95% CI [0.22, 0.78]), and closed-form output-level probes on a spectral FNO roster (a transport/phase fault evades both MRs at 0/24, Wilson 95% CI [0.00, 0.14]) and a pointwise Burgers PINN roster (a cos(πx) perturbation evades both MRs at 0/6, Wilson 95% CI [0.00, 0.39]). Trained mutants and closed-form probes are not interchangeable evidence; this reproduces the pattern across four families rather than proving it general across architectures.
