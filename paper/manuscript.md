...
> Draft status: v0.5 Phase-4 clarity revision, 2026-06-11.
> Cross-SUT and broad generalization claims remain blocked; scoped MGN, PINN, baseline, and fault-catalog artifacts are reported only within their evidence boundaries.
...
The case-study evidence is scoped and artifact-backed. On the original MeshGraphNets checkpoint, node permutation holds to machine precision; mirror-y is a bounded within-SUT frame-level OOD-stress result that failed on 10 of 10 recorded eval frames (median relative L2 0.737, median V/floor 3.96); absolute conservation remains deferred while the reference-relative non-regression guard passes. Two controls calibrate the interpretation: rollout accuracy has median relative L2 0.0216, so mirror-y is about 34 times the in-distribution error, and a provably symmetric out-of-sample mesh still violates exact mirror-y (relative L2 1.10). The same conclusions replicate across a K=6 MeshGraphNets roster; the P1 divergence operator has measured O(h) behavior; a K=6 PINN roster transfers the conservation rewrite while heat-equation mirror-y remains mixed; and a 36-entry unified fault catalogue reports detector precision/recall and effect-size statistics. The evidence remains one bounded within-SUT/cross-family artifact package.
...
**RQ4 - Case-study evidence.** In a MeshGraphNets-family cylinder-flow case study, what evidence does the rubric-gated asset workflow add relative to expert MR design, LLM-assisted candidate generation, generic MR-generation scope contrasts, and rollout-accuracy diagnostics? The current evidence answers this only through scoped comparators: rollout accuracy, LLM-simulated expert MR elicitation, one-shot LLM generation, and a generic-MR template contrast.
...
5.7). This section reports only scoped evidence whose artifacts are committed
and validated: within-SUT pilots, K=6 MGN/PINN rosters, baseline comparators,
and the unified fault catalogue.
...
| PC12-operator-floor-resolution | Observed | `operator-floor-sweep-extended/operator_floor_extended_report.json` | Log-log slope 0.984, 95% CI [0.975, 0.992] (interior 0.989, CI [0.982, 0.997]), R² = 0.9999 over nine resolutions on one symmetric structured mesh family with one analytic field; calibrates the admissibility predicate, does not generalize across mesh geometries. |
...
The PC# identifiers above are paper-level claims. The single source of truth for runtime-evidence claims is the runtime claim ledger (claims C1–C16); each paper claim maps to it as: PC1→C4-rubric-decision-coverage; PC2→C1-fixture-asset-path and C4-rubric-decision-coverage; PC3→C3-baseline-comparison; PC4→C2-real-sut-verdicts; PC5→C7-conservation-diagnostic; PC6→C6-mirror-y-ood-stress; PC7→ no runtime-evidence claim (a method/ethics process boundary); PC8→C8-rollout-accuracy-baseline; PC9→C9-mirror-y-exact-symmetric-mesh; PC10→C10-seeded-fault-detection; PC11→C11-multicheckpoint-replication; PC12→C12-operator-floor-resolution; PC13→C13-fault-detection-robustness and C16-unified-fault-catalog-statistics. The ledger's C5-precondition-check underlies the precondition gate described in the method section.
...
- **Geometric MR (out-of-relation-domain, approximate OOD-stress frame rate).**
  ...
  frame-level OOD-stress result under an approximate reflection: one SUT, one checkpoint, one MR, one eval trajectory. The 10 frames are consecutive states of one trajectory
...
- **Rollout-accuracy diagnostic (accuracy comparator on the same SUT).** On the same
  ...
  answer different validation questions on this surrogate; it is
...
- **Exact mirror-y on a symmetric mesh (admissible relation, out-of-sample test).** To
  ...
  calibrated in-distribution magnitude. And this is one input on one mesh. The 1.10 magnitude is also larger than the
...
- **Seeded-fault detection (do the MRs catch known faults?).** We re-implemented, in
  ...
  one SUT, one checkpoint, one injected-fault catalogue.
...
To address the single-checkpoint objection without converting the study into a cross-SUT claim, the five within-SUT measurements are replayed on a K=6 MeshGraphNets roster: four seed replicas plus one wider and one deeper configuration variant, all trained from the same read-only trainer and DeepMind cylinder-flow data. Per-checkpoint manifests record checkpoint hashes and runner outputs (see §5.1).

The compact reading is: node-permutation equivariance is exact on all six checkpoints; mirror-y OOD-stress has seed-family median relative L2 CI [0.743, 0.804] with S4/S5 in the same band; the provably symmetric exact-mirror-y check fails on every checkpoint; the reference-relative conservation ratio remains near one (CI [1.007, 1.011]); and rollout relative L2 stays near 0.022 (CI [0.0217, 0.0224]). Thus the mirror-y violation remains about 35 times the in-distribution one-step error across the roster. This is a cross-checkpoint / cross-configuration replication within one architecture family and one dataset, not an independent cross-SUT sample.
...
**Unified catalogue and statistics.** Phase 3 merges 10 executed canonical MGN mutants, 2 executed adversarial MGN mutants, and 24 closed-form output-level PINN probes into a 36-entry unified fault catalogue (see §5.1). The artifact reports precision/recall with Wilson intervals: node-permutation 1.00/1.00, conservation 1.00/0.78, and mirror-y 0.92/0.49; it also reports Wilcoxon and Cliff's δ effect-size summaries, including PINN MR-B diffusion-vs-Burgers δ = 0.78 (large) with paired Wilcoxon p = 0.5 at n = 3. The PINN entries are closed-form probes with no retraining, not retrained PINN mutant checkpoints.

### 5.6.4 LLM and generic-MR baselines

**Expert-MR baseline.** Three expert-LLMs proposed 25 candidate MRs (24 unique) without access to the rubric. Three independent rater models then applied the four-condition predicate by majority vote: 4 candidates were retained, 2 downgraded to OOD-stress, 13 rejected, and 6 deferred. The 76% rejection/deferral rate and the absence of any novel retained candidate show an admissibility gap between unguided elicitation and validity-gated MR construction; this is an automated LLM-simulated expert baseline, not a human-expert benchmark.
...
The PINN extension checks whether the predicate and MR rewrites can be carried outside the MeshGraphNets implementation. The K=6 PINN roster contains three 2D Burgers seeds and three 2D heat-equation seeds, with checkpoints and reports committed (see §5.1). MR-A remains vacuous by construction for pointwise MLP PINNs; the two non-trivial MR checks are mirror-y and reference-relative conservation. MR-B passes on Burgers (mean ratio 0.615, CI [0.446, 0.890]) but is mixed on heat (mean 1.682, CI [0.853, 2.795]). MR-C passes on all six PINNs: Burgers mean 1.007 (CI [1.000, 1.011]) and heat mean 0.992 (CI [0.971, 1.012]). This is a two-PDE seed roster, not a PINN-vs-MGN benchmark or a general claim across PINN architectures, PDEs, or training regimes.

### 5.7 Still blocked

The canonical block list is unchanged: cross-SUT or geometry-independent pass/fail rates; comparative superiority over baselines; general or real-world fault-detection rates; validated localization; runtime; reliability; model accuracy; and broad external validity. The executed comparators (rollout, seeded-fault, expert-MR, generic-MR, and LLM-MR) are scoped diagnostics. The K=6 MGN and K=6 PINN rosters expand the artifact package but do not lift the cross-SUT block.
...
**Conclusion validity.** Small sample sizes, multiple MRs, and many transformation bins can produce unstable conclusions. The mirror-y evidence has a bounded within-SUT frame-level OOD-stress rate, while conservation and node permutation remain pilots.
...
This paper presents domain-validity-gated MR identification as an auditable oracle-free testing workflow for MeshGraphNets-family cylinder-flow surrogates. The current evidence supports the scoped within-SUT pilots of Section 5.3 (node-permutation, mirror-y OOD-stress, conservation diagnostic, rollout-accuracy comparator, exact mirror-y on a symmetric admissible mesh, and seeded-fault detection over a 10-mutant catalogue), replicated across a K=6 multi-checkpoint roster of one MeshGraphNets architecture family on one dataset (Section 5.4) — the verdicts reproduce with tight Wilson and bootstrap 95% confidence intervals — together with an empirical calibration of the P1 discrete-divergence operator floor that gates the admissibility predicate (slope 0.984, 95% CI [0.975, 0.992]; Section 5.5), a within-family fault-detection robustness study with severity sweeps that bound where the present MR set is structurally insensitive (Section 5.6), and a K=6 PINN roster that checks the predicate and two non-trivial MR rewrites across Burgers and heat-equation PINNs. The central claim remains methodological: physically meaningful SciML MRs require explicit validity conditions, executable assets, raw evidence records, and relation-level verdicts; the K=6 MGN replication, PINN roster, and operator-floor calibration are the present study's bounded evidence that the workflow's verdicts hold beyond a single checkpoint and that the admissibility-predicate gate it relies on has a measurable basis.
