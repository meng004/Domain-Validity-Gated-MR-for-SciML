...
- **Manuscript authority:** the authoritative paper source is always
  `manuscript/`, especially `manuscript/main.tex` and
  `manuscript/references.bib`. Venue packages under `venues/` and generated
  upload packages under `submission/` must be derived from `manuscript/`.
  Do not promote a generated package, root-level `source/`, or venue-specific
  copy to the paper source of truth.
...
3. **Manuscript** — `manuscript/main.tex` is the authoritative LaTeX source
   for submitted paper text; `manuscript/references.bib` is the authoritative
   bibliography source; `manuscript/figures/` contains the authoritative
   manuscript figures. `manuscript/manuscript.md` is a prose draft/audit
   companion, not the source used to build a venue package unless explicitly
   requested.
