...
分两大阶段，**严格按序**；每阶段末有 verification gate（§6）+ commit + 向人类报告。

```
阶段 1（全本地 CPU，无 GPU/凭据）── 先做、必做
  ├─ MVP-A   PointMLP 端到端 seeded-fault（第二个收敛架构族）
  ├─ MVP-B/C 三臂互补矩阵 + ≥20 fault + Wilson CI + knife-edge 曲线
  └─ EXT-2   operator-floor ≥2 mesh 拓扑一致性
  → verification gate → commit → 报告 → 【停，等人类确认是否进阶段 2】

阶段 2（需 GPU）── 仅在人类确认后做
  └─ EXT-1   airfoil 训到收敛（不同物理第三 SUT），重跑现有端到端 runner
  → verification gate → commit → 报告
```

> **为什么阶段 1 后要停**：阶段 1 全部零 GPU/零凭据、且无论冲几区都净收益；阶段 2 的 airfoil GPU 训练昂贵且有"训不收敛"风险。先交付阶段 1 证据、让人类（在有 LLM 网关凭据的本地）重跑评审 panel 看分数是否上移，**验证有效后**再投 GPU。不要自作主张直接跑阶段 2。
...
### 任务 EXT-1 — airfoil 训到收敛（不同物理第三 SUT）【需 GPU · 阶段 2 · 人类确认后才做】
...
- **阶段 1 结束后停**：报告完等人类明确说"继续阶段 2"再碰 GPU（§4 已说明原因）。
