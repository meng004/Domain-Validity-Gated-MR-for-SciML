...
class ProductionSutFeasibilityPlanTest(unittest.TestCase):
    ...
    def test_plan_task1_checkboxes_are_updated_without_advancing_result_tasks(self) -> None:
        ...
        self.assertIn("tools/run_physicsnemo_mgn_smoke_workflow.py", text)
        self.assertIn("Task 4--5 remain blocked", text)
    ...
    def test_claim_ledger_only_adds_blocked_feasibility_entry(self) -> None:
        ...
        self.assertIn("Full-scale production workflows have not produced raw outputs yet", text)
        self.assertIn("Do not claim full-scale PhysicsNeMo/AeroGraphNet/DoMINO primary workflow results", text)
