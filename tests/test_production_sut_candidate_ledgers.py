...
class ProductionSutCandidateLedgersTest(unittest.TestCase):
    ...
    def test_plan_and_claim_ledger_record_task2_without_advancing_workflows(self) -> None:
        ...
        self.assertIn("tools/run_physicsnemo_mgn_smoke_workflow.py", plan)
        self.assertIn("Task 4--5 remain blocked", plan)
        claim_text = CLAIM_LEDGER.read_text(encoding="utf-8")
        self.assertIn("candidate ledgers classify", claim_text)
        self.assertIn("planned MR", claim_text)
        self.assertIn("No full-scale production MR has been", claim_text)
