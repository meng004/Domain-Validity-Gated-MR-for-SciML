...
def main() -> int:
    ...
    if "elsarticle" not in content:
        errors.append("documentclass is not elsarticle")
    ...
    keywords = re.search(
        r"\\begin\{keyword\}(.*?)\\end\{keyword\}", content, re.DOTALL
    )
    if not keywords:
    ...
    highlights = re.search(
        r"\\begin\{highlights\}(.*?)\\end\{highlights\}", content, re.DOTALL
    )
    if highlights:
        items = [
        ...
        warnings.append("no highlights environment in main.tex; ensure separate highlights file exists")
    ...
    if "author biography" in content.lower() or "author biographies" in content.lower():
        errors.append("author biographies should be a separate editable file, not in main.tex")

    if re.search(r"\\includegraphics(?:\[[^\]]*\])?\{[^}]*[/\\][^}]+\}", content):
        warnings.append(
            "main.tex references graphics in subfolders; flatten paths for Editorial Manager source zip"
        )
