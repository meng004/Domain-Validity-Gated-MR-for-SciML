...
- **稿件 tex 源**（`main.tex` / `supplementary.tex` / `references.bib` / `figures/*.pdf`）：在 `source/`
...
- 必备声明（已写入 `source/main.tex`）：CRediT、Competing Interest、Generative AI、Data Availability、Funding

## 同步规则

改动 `source/` 后重新生成 IST 镜像：
