...
Target package: `submission/JSS_regular_20260704/`.
...
|---|---|---|
| JSS precheck | Pass | `tools/precheck_jss.py submission/JSS_regular_20260704/source/main.tex` passed. |
| Main PDF build | Pass | Final log reports `Output written on main.pdf (35 pages, 414665 bytes).`. |
...
| Highlights | Pass | 5 separate editable highlights; max length 62 characters including bullet markers. |
...
| Unique LaTeX source | Pass | Repository-level `source/` is the canonical source; package-level `source/` contains generated upload copies of `main.tex` and `supplementary.tex`. |
| Source archive | Pass | `jss_latex_source.zip` is flat and has no temporary, backup, log, nested, or compiled-output entries. |
| Package archive | Pass | `jss_submission_package_20260704.zip` includes the submission materials and no temporary, backup, log, or auxiliary files outside `review/`. |
| PDF/source consistency | Pass | The top-level `main.pdf` and `supplementary.pdf` are copied from the freshly compiled files in `source/`. |
